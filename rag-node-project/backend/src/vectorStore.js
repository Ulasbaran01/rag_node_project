...
import fs from "fs";
import { termFrequency, cosineSimilarity, lexicalOverlapScore, combinedSimilarity } from "./chunker.js";
...
export class VectorStore {
  ...
  search(query, topK = 5) {
    ...
    // Use inverted index to find candidate chunks: exact term matches, plus
    // (when no exact match exists) terms that share a substring/prefix
    // relationship with the query term — otherwise compound words like
    // "esyslauncherpremium" would never surface for a query like "esys",
    // since the lexical-overlap scoring below would never get a chance to run.
    const candidateIndices = new Set();
    for (const term of queryTerms) {
      const exact = this._invertedIndex.get(term);
      if (exact) {
        for (const idx of exact) candidateIndices.add(idx);
        continue;
      }
      for (const [indexedTerm, indices] of this._invertedIndex) {
    ...
    // Score only candidates instead of all rows: blend cosine similarity
    // with lexical overlap so compound words / partial matches still surface.
    const scored = [];
    for (const idx of candidateIndices) {
      const row = this._rowCache[idx];
      const cosine = cosineSimilarity(queryTf, row.tf);
      const lexical = lexicalOverlapScore(queryTerms, row.content, row.tf.keys());
      const score = combinedSimilarity(cosine, lexical);
      if (score > 0) {
